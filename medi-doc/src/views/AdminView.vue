<template>
  <div class="home">
    ...
    <div>
      ...
      <h3>les rdv :</h3>
      <ul v-show="i.status == 'en attente'" v-for="(i, index) in rdv" :key="index">
        <li> {{ i.date }} <br>
          {{ i.time }} <br>
          {{ i.patientID }}<br></li>
        <form @submit.prevent="() => affecter(i, index)">
          <select v-model="selectedMedecin[index]" required>
            <option v-show="medecin.role == 'medecin'" v-for="medecin in userData" :value="medecin.uid">{{ medecin.email }}
            </option>
          </select>
